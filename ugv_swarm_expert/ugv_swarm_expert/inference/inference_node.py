...
from ugv_swarm_expert.env.safety_supervisor import SafetySupervisor
...
class UGVInferenceNode(Node):
    def __init__(self) -> None:
        ...
        self.imu_topic_template = str(self._declare_parameter_value("imu_topic_template", "/{agent}/imu"))
        ...
        self.model.eval()
        self._safety_supervisor = SafetySupervisor(logger=self.get_logger())
    ...
    def timer_callback(self) -> None:
        ...
        try:
            ...
            cmd = self._safety_supervisor.apply_safety_policy(
                float(velocity[0]), float(velocity[1]), self._latest_scan
            )
    ...
    def _create_ros_interfaces(self) -> None:
        ...
        local_imu_topic = self._format_topic(self.imu_topic_template, self.robot_namespace)
        ...
        self._subscriptions.append(
            self.create_subscription(Imu, local_imu_topic, self._imu_callback, scan_qos)
        )
        self._subscriptions.append(
